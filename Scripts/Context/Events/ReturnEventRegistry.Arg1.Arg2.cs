...
{
    private static readonly Dictionary<IEventContext, Dictionary<string,Func<TArg1,TArg2,TReturn>>> _eventDictionary 
        = new Dictionary<IEventContext, Dictionary<string,Func<TArg1,TArg2,TReturn>>>();
    ...
    public static void Register(IEventContext main, string key,Func<TArg1,TArg2,TReturn> action)
    {
        if(!ContainsEvent(main,key)) Install(main,key);
        _eventDictionary[main][key] += action;
    }

    public static void Unregister(IEventContext main, string key,Func<TArg1,TArg2,TReturn> action)
    {
        _eventDictionary[main][key] -= action;
    }

    public static TReturn Raise(IEventContext main, string key,TArg1 arg1,TArg2 arg2)
    {
        if (!ContainsEvent(main,key)) return default;
        return _eventDictionary[main][key].Invoke(arg1,arg2);
    }

    public static TReturn TryRaise(IEventContext main, string key, TArg1 arg1, TArg2 arg2)
    {
        if (!ContainsEvent(main,key)) return default;
        return _eventDictionary[main][key].Invoke(arg1,arg2);
    }
