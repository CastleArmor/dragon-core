...
namespace Dragon.Core
{
    ...
    {
        private static readonly Dictionary<IContext, Dictionary<string,Func<TArg1,TReturn>>> _eventDictionary 
            = new Dictionary<IContext, Dictionary<string,Func<TArg1,TReturn>>>();
        ...
        public static void Register(IContext main, string key,Func<TArg1,TReturn> action)
        {
            if(!ContainsEvent(main,key)) Install(main,key);
            _eventDictionary[main][key] += action;
        }

        public static void Unregister(IContext main, string key,Func<TArg1,TReturn> action)
        {
            if (!ContainsEvent(main, key)) return;
            _eventDictionary[main][key] -= action;
        }

        public static TReturn Raise(IContext main, string key,TArg1 arg1)
        {
        ...
        public static TReturn TryRaise(IContext main, string key,TArg1 arg1)
        {
            if (!ContainsEvent(main,key)) return default;
            return _eventDictionary[main][key].Invoke(arg1);
        }
